      ...
      <div className={classNames("rounded-[2rem] border p-8 md:p-10", darkMode ? "border-slate-800 bg-slate-900/70" : "border-slate-200 bg-white")}>
        <div className="grid gap-8 md:grid-cols-[1.1fr_0.9fr] md:items-center">
          <div>
            ...
            <p className={classNames("mt-4 max-w-2xl leading-7", mutedText)}>I'm always happy to chat about exciting ideas, projects and opportunities to collaborate.</p>
          </div>
          <div className="flex flex-col gap-3">
            <a href={`mailto:${profile.email}`} aria-label={`Email ${profile.email}`} className={classNames("inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-bold transition", darkMode ? "bg-sky-300 text-slate-950 hover:bg-sky-200" : "bg-slate-950 text-white hover:bg-slate-800")}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><rect width="20" height="16" x="2" y="4" rx="2" /><path d="m22 7-10 6L2 7" /></svg><span>Email me</span></a>
